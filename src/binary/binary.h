...
public:
  ...
  Binary(std::string path)
  {
    ...
    const IMAGE_NT_HEADERS* nt_headers = get_ptr<IMAGE_NT_HEADERS>(dos->e_lfanew);
    ...
    {
      std::stringstream _;
      _ << "Entry Point Ptr: 0x" << std::hex << nt_headers->OptionalHeader.AddressOfEntryPoint;
      Logger::get()->log(_.str(), "BinaryAnalyzer");
    }
  }
  ...
  std::vector<resource_t> get_exports()
  {
    ...
    IMAGE_DATA_DIRECTORY p_exports = nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT];
    const IMAGE_EXPORT_DIRECTORY* exports = get_ptr<IMAGE_EXPORT_DIRECTORY>(p_exports.VirtualAddress);

    DWORD numberOfNames = exports->NumberOfNames;
    const DWORD* exportAddressTable = get_ptr<DWORD>(exports->AddressOfFunctions);
    const DWORD* nameOrdinalsPointer = get_ptr<DWORD>(exports->AddressOfNameOrdinals);
    const DWORD* exportNamePointerTable = get_ptr<DWORD>(exports->AddressOfNames);

    std::vector<resource_t> cleaned_exports;
    // FIXME once ur not sleep deprived and without the need to push the latest codebase to the repo
    /*for (int i = 0; i < numberOfNames; ++i) {
      char* name = (char*)(reinterpret_cast<const uint8_t*>(m_view) + exportNamePointerTable[i]);
      resource_t resource;
  ...
  template<typename T>
  const T* get_ptr(size_t address) const {
  ...
  {
    if (!m_view || address >= m_file_size || !size) return {};

    if (address + size > m_file_size)
      size = m_file_size - 1;

    std::vector<unsigned char> ret(size);  // Direct construction with size
    memcpy(ret.data(), static_cast<const unsigned char*>(m_view) + address, size);
    return ret;
  }
  ...
  const IMAGE_NT_HEADERS* get_nt()
  {
    ...
    if (nt->Signature != IMAGE_NT_SIGNATURE) {
      Logger::get()->log("Error: Not a valid PE binary. (NT header)", "BinaryAnalyzer");
      close();
      return {};
    }
    return nt;
  ...
  size_t get_entrypoint()
  {
    ...
    if (nt->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC)
      entry_point = reinterpret_cast<const IMAGE_NT_HEADERS32*>(nt)->OptionalHeader.AddressOfEntryPoint;
    else if (nt->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR64_MAGIC)
      entry_point = reinterpret_cast<const IMAGE_NT_HEADERS64*>(nt)->OptionalHeader.AddressOfEntryPoint;
